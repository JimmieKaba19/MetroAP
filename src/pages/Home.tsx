...
export default function Home() {
  ...
  const label = (text: string) => (
    <div style={{ fontSize: R.labelSize, color: T.textSub, letterSpacing: 1, marginBottom: '0.5rem' }}>{text}</div>
  )

  // ── Shared controls (used in both layouts) ────────────────────────────────

  const CorridorSelector = (
    <div style={{ marginBottom: R.sectionGap }}>
      {label('ROUTE CORRIDOR')}
      <button onClick={() => setShowCorridorMenu(v => !v)} style={{ width: '100%', background: T.surface, border: `1px solid ${color}55`, borderRadius: R.cardRadius, padding: R.cardPadding, display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', textAlign: 'left' }}>
  ...
  const StopChips = (
    <div style={{ marginBottom: R.sectionGap }}>
      ...
      {/* flexWrap so chips never push outside the viewport on narrow screens */}
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
  ...
  const ReportButton = (
    <button onClick={() => setShowFeedback(true)} style={{ width: '100%', marginBottom: R.sectionGap, background: T.surface, border: `1px dashed ${color}66`, borderRadius: R.cardRadius, padding: R.cardPadding, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, cursor: 'pointer', transition: 'all 0.2s ease', minHeight: 48 }}>
      ...
      {!R.isTablet && R.tier !== 'compact' && <span style={{ fontSize: R.subSize, color: T.textMuted }}>· ma3route style</span>}
    </button>
  )
  ...
  // ── Feedback panel (full screen on both layouts) ───────────────────────────
  if (showFeedback) {
    return (
      <FeedbackPanel
  ...
  if (R.isTablet) {
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', animation: 'slideUp 0.35s ease', alignItems: 'start' }}>
